...
# %%
# Constant for the maximum number of output rows
MAX_OUTPUT_ROWS = 100

# %%
def main():
    """
    Main function to launch the bioacoustic bird detection tool.
    It sets up the Gradio interface for annotation and validation of bioacoustic recordings.
    """
    ...
    with gr.Blocks(theme=gr.themes.Soft()) as demo:
        ...
        with gr.Column(visible=True) as init_col:
            gr.Markdown("### Choose one task to start:")
            with gr.Row():
                ann_init_but = gr.Button("Click to start annotation!")
                val_init_but = gr.Button("Click to start validation!")
        
        # Initial column for task selection
        with gr.Column(visible=False) as ann_tab:
        ...
        # Validation tab config
        with gr.Column(visible=False) as val_tab:
        ...
        # %% # Init buttons for annotation and validation
        ann_init_but.click(tab_init, outputs=[init_col, ann_tab])
        val_init_but.click(tab_init, outputs=[init_col, val_tab])

        # %% # Annotation buttons and actions
        # Load the data
        ann_data_fetch_but.click(ann_logger.load_data, 
        ...
        # Register the name
        ann_name_reg_but.click(ann_logger.register_ann_file, 
                               inputs=[ann_path, ann_name], 
                               outputs=[ann_name, start_but])
        # Start the annotation
        start_but.click(ann_logger.start_annotation, 
